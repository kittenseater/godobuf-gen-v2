...
std::string rfu_int32() {
    std::string out;
    Test1 m;
    m.add_rfu_int32(1234);
    m.add_rfu_int32(5678);
    m.SerializeToString(&out);
    return out;
}
...
std::string rfu_int64() {
    std::string out;
    Test1 m;
    m.add_rfu_int64(1234);
    m.add_rfu_int64(5678);
    m.SerializeToString(&out);
    return out;
}
...
void exec(bool save_to_file, const std::vector<std::string>& tests_names) {
    const std::string out_dir_path = "testout2/";
    ...
    for (auto& test_name : tests_names) {
        if (test_cases.find(test_name) != test_cases.end()) {
            ...
            if (save_to_file) {
                std::string filename = out_dir_path + test_name + ".v2ref";
